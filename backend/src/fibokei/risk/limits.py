...
def get_risk_limits() -> dict:
    ...
    Portfolio design: drawdown monitoring is the PRIMARY risk control.
    Position count limits are NOT the primary gate — all bots can open
    simultaneously provided drawdown limits are not breached.

    max_open_trades: emergency backstop only (default 100).
    max_per_instrument: concentration limit per instrument (default 5).
    """
    return {
        ...
        # Emergency backstop — not the primary gate (drawdown is)
        "max_open_trades": _int("FIBOKEI_MAX_OPEN_TRADES", 100),
        # Concentration limit: max simultaneous positions on one instrument
        "max_per_instrument": _int("FIBOKEI_MAX_PER_INSTRUMENT", 5),
        ...
        "fleet_max_bots_per_instrument": _int("FIBOKEI_FLEET_MAX_BOTS_PER_INSTRUMENT", 10),
        "fleet_max_total_positions": _int("FIBOKEI_FLEET_MAX_TOTAL_POSITIONS", 100),
        "fleet_max_exposure_per_instrument": _int("FIBOKEI_FLEET_MAX_EXPOSURE_PER_INSTRUMENT", 10),
